...
- **Thread 8:** the redline-side `IFinancialExtractor` adapter that reads them,
  over the additive `GET /financial-extractions/{source_doc_id}` read endpoint
  this overlay serves.
...
| `GET` | `/financial-extractions/{source_doc_id}` | Read a document's extracted figures (Thread 8 read seam — `DocumentExtractionsRead`). A document with no extractions is a **200 + empty list**, not a `404`. Mounted via `build_extractions_router`. |
...
2. `app.include_router(build_router(get_session))` **and**
   `app.include_router(build_extractions_router(get_session))` on the backend
   app; reuse Numbatch's async session dependency.
...
- `test_extraction_read_api.py` — **the Thread 8 read seam**: `GET
  /financial-extractions/{source_doc_id}` serves a document's figures as JSON
  (numeric + description-fallback rows), returns all rows for a document, and
  treats an unknown document as an empty `200`, not a `404`.
