...
const SERVER_INSTRUCTIONS = [
  ...
  "",
  "SIZE BEFORE YOU READ. discover_corpus_shape tells you how big a run, an asset or",
  "a document is, and for one document which element kinds and printed pages it",
  "holds — for almost nothing, because it counts rows without decoding them. How",
  "much you can afford to read is your judgement, not redline's: it depends on the",
  "window you have left and whether you are reading one document or twenty, and",
  "redline knows neither. Call it first and choose a limit and a filter from what it",
  "says. A capped payload means you skipped this step.",
  "",
  "WHAT IS HERE. discover_corpus_shape sizes a corpus, a run or a document.",
  "list_documents is the catalogue: it names the documents in a run from metadata",
  "alone. read_extraction_elements returns a document's ordered element",
  ...
  "where to look, never as an extracted fact you can cite. The same holds for every",
  "number discover_corpus_shape returns: counts and tallies are redline's",
  "arithmetic over row metadata, so they size a read and are never themselves a",
  "fact about a document's contents.",
].join("\n");
