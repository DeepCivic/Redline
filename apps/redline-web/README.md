...
| `src/lib/review-grid.ts` | The **review grid brain** (Thread 12): turns the `BuildEvaluationTable` output (one `ProcurementResponse` per (group, document, matched requirement)) into typed, sortable, filterable rows. Currency routes through Wayfinder's `typedDisplayCell` so it stays a real number — it sorts numerically and exports numeric (Thread 14). Each row carries source provenance (document / element / page / chunk) for a deep-link. |
...
| `src/lib/pricing-pivot.ts` | The **pivot brain** (Thread 13): rolls the `ProcurementResponse[]` up per brand, per requirement, or brand×requirement, summing/averaging the real-number `estimateAud` — mirroring Wayfinder's `computePivot` algorithm over redline's own domain type. |
...
| `src/lib/excel-export.ts` | The **export brain** (Thread 14): pure builders that turn the review grid + pricing pivots into `write-excel-file` sheet data — numeric currency cells, a source hyperlink per row, one `Review` sheet + one sheet per pivot — plus `exportEvaluationXlsx`, a lazy `write-excel-file/browser` trigger. Reuses Wayfinder's XLSX cell shape (plan §9) so currency stays a real numeric cell. |
