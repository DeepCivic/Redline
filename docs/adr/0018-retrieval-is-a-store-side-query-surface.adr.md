...
- **Ships now — the graph:** womblex's `enrich` output (`entities`, `graph_edges`,
  ...
  **Operational reality, updated 2026-08-06:** enrich is now **enabled**
  (`infra/womblex/redline.yaml`: `enrichment.enabled: true`) and has been run
  against a real corpus, producing 133 entities and 1,771 graph edges over 4
  documents. It is real Isaacus spend (`kanon-2-enricher`), and it is a separate
  `run-stage --stage enrich` pass that must follow `chunk` — chunks are a
  non-strict input, so running it first still succeeds and lands every entity
  with `chunk_index = -1`, a graph that cannot be joined to chunk text.

  `linking.enabled` stays **false**, and is a different thing: the graph comes
  from `enrich`, while `link` writes `*.entity_links.parquet`, which nothing in
  redline reads, and its preflight hard-fails without a reference register a
  tender corpus has nothing to point at.

  The graph is on object storage; **loading it into `redline_` tables is still
  open** and is a delivery decision, not a change to this ADR.
