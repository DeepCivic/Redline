...
- **Date**: 2026-08-01
- **Status**: **Accepted and implemented.** The mount described below is built and
  runs. `services/wayfinder` is a submodule of the fork on the
  `redline-integration` branch; the `evaluation` tRPC router, the
  `container-redline.ts` module factory, the `"use client"` surfaces and the
  `app/(user)/evaluations/[id]/{grouping,review,pivots,documents}` route pages
  exist in the fork's `apps/web`, and the review grid, pricing pivots, Excel
  export and document view are served through them, with six
  `apps/web/e2e/redline-*.spec.ts` Playwright specs against the served app. The
  version pins below (upstream 0.20.0) record the state at the time of the
  decision, not the current pin.
...
green under `./validate.sh`. What had never existed, at the time of this
decision, was the thing that *serves* them — the only genuinely missing piece
of the vertical then. (It has since been built, in the fork's `apps/web`; see
Status.)

Reading Wayfinder at the then-current pin (0.20.0, `60767f4`) settled how that
piece should land. Wayfinder ships a full **extraction feature** entirely inside
its own tree — `apps/web/src/components/extraction/result-grid-model.ts` (the pure
...
- The existing Playwright specs (`apps/redline-web/e2e/`) finally have a real
  ...
  `CLAUDE.md`. (Realised: the specs now live at `apps/web/e2e/redline-*.spec.ts`
  in the fork, against the served mount.)
...
- redline's packages must be resolvable from the fork's `apps/web`. Wiring two
  ...
  when the mount was built, not here.
