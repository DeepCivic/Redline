...
// calls its POST /embeddings/query endpoint, which
// declares the producing model and L2-normalises the vector exactly as the
// chunk embedder does.
//
// The retrieval stage embeds a topic definition with this port and
...
export interface ITextEmbedder {
  ...
  // is only ever compared, never stored. Blank text is an
