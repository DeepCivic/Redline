...
// no requirement — attribution is this extractor's job, and the interim rule is:
...
// A document's spans are summed once and land on that single row, so the per-brand
// pivot totals them without double-counting a document that matched >1 requirement.
...
const request = (
  over: Partial<FinancialExtractionRequest> = {},
): FinancialExtractionRequest => ({
  evaluationId: "eval-1",
  responseGroupId: "g-acme",
  documentIds: ["doc-a"],
  matchedRequirements: [
    { documentId: "doc-a", requirementId: "req-price", confidence: 0.86 },
  ],
  ...over,
});
...
describe("MoneySpanFinancialExtractor — real AUD from the money spans", () => {
  ...
  it("carries the span's provenance element order onto the extraction", async () => {
    const store = new InMemoryMoneySpanStore([
      span({ parentElementOrder: 12, value: "50.0000" }),
    ]);
    const extractor = new MoneySpanFinancialExtractor({ moneySpanStore: store });
