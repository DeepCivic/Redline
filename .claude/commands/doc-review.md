...
| 4 | **Nothing upstream already does this** | The item rebuilds a capability in `services/womblex`. Read the submodule to be sure — do not answer from memory |
| 5 | DB changes follow conventions | Wrong prefix (must be `redline_`), camelCase columns |
| 6 | Layering respected | Domain gains a dep; application imports an adapter; app reaches into Wayfinder internals |
| 7 | Wayfinder reuse is read-only | Any plan to modify `services/wayfinder`'s tree beyond the fork surface the plan names |
...
columns that do not exist upstream. Verify against the submodule, not memory.
