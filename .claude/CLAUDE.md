...
**The context window shapes the tool surface.** A corpus is however many documents
it is and no model holds them at once. Redline does navigation (choose what
matters, from metadata alone) and retrieval (exact bytes for one narrow thing, in
small pages). Accumulating findings across documents is the client's — redline is
stateless. A tool that could return a whole corpus is one that will.

**One document at a time, against a CSV the client keeps.** Redline assumes its
client can write and re-read a CSV between calls, and shapes payloads to be
appended to one: flat scalar rows, stable column order across pages, an anchor per
row, a cursor that resolves with no server state. Document scope is **mandatory**
on every retrieval tool — only `list_runs` and `list_documents` span a run, and
they return metadata only. A page size above the cap is an **error**, not a silent
clamp. See [`docs/Redline-Status.md`](../docs/Redline-Status.md) §2.
...
|---|---|---|---|
| Planning artefact | PRD + ADR + phase doc per feature | `docs/Redline-Status.md` (what is present in §3 + what is outstanding in §4, locally numbered). **No ADRs** — decisions are made and recorded in the commit that acts on them | One-repo delivery, sequenced directly in the doc; documents never gate a build |
