# CLAUDE.md — Routing Index (procautomatr)
...
This repo implements **procautomatr**, a procurement-evaluation adapter for
**Wayfinder**. Our own packages live under `@procautomatr/*` in `packages/`.
...
- `packages/proc-domain` has **zero external dependencies**. Pure TypeScript,
  relative imports only. (Includes no import of `@rbrasier/*`.)
- `packages/proc-application` imports only `@procautomatr/proc-domain` and
  `@procautomatr/proc-shared`. No frameworks, no ORMs, no AI SDKs.
- `packages/proc-adapters` implements ports from `proc-domain`. Drizzle, HTTP
  clients (womblex/Numbatch), object storage, and the read-only reuse of
  Wayfinder's `@rbrasier/domain` typed helpers live here.
- Apps (`apps/*`) import from `@procautomatr/proc-application` and
  `@procautomatr/proc-adapters` only. Wiring lives in `lib/container.ts`.
...
- DB tables use the **`proc_` prefix** in a separate Postgres schema/DB. Columns are
...
- We **never modify Wayfinder's tree**. `vendor/wayfinder` is read-only reuse only,
  and is excluded from lint/format/build/test scope (`--filter=@procautomatr/*`).
...
| Area | Wayfinder | procautomatr | Why |
|---|---|---|---|
...
| E2E | Playwright suite exists day one (`/e2e`) | Deferred until `apps/proc-web` (Thread 11) | No UI yet; adding `/e2e` when the app lands |
...
| Scope | `@rbrasier/*` | `@procautomatr/*`, consuming `@rbrasier/*` read-only | This is an adapter, not the framework |
